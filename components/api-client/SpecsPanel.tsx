...
function EndpointCard({ endpoint, baseUrl }: { endpoint: SpecEndpoint; baseUrl: string }) {
  ...
  return (
      <div className={cn(
          'rounded-xl border transition-all overflow-hidden bg-background',
          endpoint.deprecated ? 'border-border/50 opacity-60' : 'border-border hover:border-border/80 hover:shadow-sm',
          expanded && 'border-primary/30 shadow-md shadow-primary/5 ring-1 ring-primary/10'
      )}>
        <button
                            ...
                            <div key={param.name} className="flex items-start gap-2">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  ...
                                  {param.required && <span className="text-[9px] text-rose-500 font-medium">required</span>}
                                  <span className="text-[9px] text-muted-foreground/80 bg-muted/50 px-1.5 py-0.5 rounded font-mono border border-border/50">{param.in}</span>
                                  <span className="text-[9px] text-muted-foreground/80 bg-muted/50 px-1.5 py-0.5 rounded font-mono border border-border/50">{param.type}</span>
                                </div>
                                {param.example && <div className="text-[10px] text-muted-foreground/60 mt-0.5 font-mono">e.g. {param.example}</div>}
                              </div>
                            </div>
                        ))}
                      </div>
  ...
  return (
      <div className="flex flex-col h-full bg-background">
        <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-muted/5 flex-shrink-0">
          ...
          <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/40 border border-border text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all text-xs">
          ...
          {specs.map((s) => (
              <button
                  ...
                  className={cn(
                      ...
                      s.id === activeSpecId ? 'bg-primary/10 text-primary shadow-sm border border-primary/20' : 'text-muted-foreground hover:text-foreground hover:bg-muted/40'
                  )}
              >
                <Layers className="size-3" />
                {s.name}
                <span className="text-[9px] bg-muted/60 text-muted-foreground px-1.5 py-0.5 rounded-full border border-border/50">{s.endpoints.length}</span>
              </button>
          ))}
        </div>
              ...
              <div className="flex items-center gap-6 px-5 py-3 border-b border-border bg-background flex-shrink-0 overflow-x-auto shadow-sm z-10">
                ...
                <div className="relative ml-auto flex-shrink-0">
                  ...
                  <input
                      ...
                      className="bg-muted/20 border border-border rounded-lg pl-8 pr-3 py-1.5 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-primary/50 focus:bg-muted/40 transition-all w-56"
                  />
                </div>
              </div>

              <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6 bg-muted/5">
                {Object.keys(tagGroups).length === 0 ? (
                    ...
                    Object.entries(tagGroups).map(([tag, endpoints]) => (
                        <div key={tag}>
                          <button onClick={() => toggleTag(tag)} className="flex items-center gap-2 mb-3 group w-full">
                            {isTagExpanded(tag) ? <ChevronDown className="size-3.5 text-muted-foreground" /> : <ChevronRight className="size-3.5 text-muted-foreground" />}
                            <span className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors">{tag}</span>
                            <span className="text-[10px] text-muted-foreground/60 bg-muted/40 px-2 py-0.5 rounded-full border border-border/50">
